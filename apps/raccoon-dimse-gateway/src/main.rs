...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    let config = raccoon_platform_orchestration::platform::config::load_dimse_gateway_config()?;
    ...
    let app = raccoon_platform_orchestration::app::build_dimse_gateway_app(&config).await?;
