...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    let config = raccoon_platform_orchestration::platform::config::load_dicomweb_gateway_config()?;
    ...
    let app = raccoon_platform_orchestration::app::build_dicomweb_gateway_app(&config).await?;
