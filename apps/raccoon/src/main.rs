...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    let config = raccoon_platform_orchestration::platform::config::load_monolith_config()?;
    ...
    let app = raccoon_platform_orchestration::app::build_monolith_app(&config).await?;
