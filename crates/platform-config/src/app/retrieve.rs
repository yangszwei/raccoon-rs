...
/// Top-level configuration for the retrieve gRPC service.
#[derive(Debug, Default, Deserialize)]
#[serde(default)]
pub struct RetrieveServiceConfig {
    ...
    /// gRPC server listener settings for the retrieve service.
...
impl RetrieveServiceConfig {
    /// Load configuration from configured sources.
    pub fn load() -> Result<Self, ConfigError> {
        Config::builder()
            .add_source(File::with_name("config/retrieve").required(false))
            .add_source(File::with_name("retrieve").required(false))
            .add_source(Environment::with_prefix("RACCOON_RETRIEVE").separator("__"))
            .build()
...
mod tests {
    ...
    use super::RetrieveServiceConfig;

    #[test]
    fn default_constructs_with_shared_component_defaults() {
        let config = RetrieveServiceConfig::default();
    ...
    fn deserializes_top_level_components_from_example_config() {
        let config: RetrieveServiceConfig = Config::builder()
        ...
        assert_eq!(config.app.name, "raccoon-retrieve");
        assert_eq!(config.filesystem.root.to_string_lossy(), "data");
        assert_eq!(config.grpc.bind_address, "127.0.0.1:50054");
    }

    fn example_config_path() -> std::path::PathBuf {
        Path::new(env!("CARGO_MANIFEST_DIR")).join("../../config/retrieve.example.toml")
    }
}
