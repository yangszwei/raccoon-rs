...
/// Postgres-backed implementation of [`QueryRepository`].
///
#[derive(Clone, Debug)]
...
impl PostgresReadRepository {
    ...
    /// Opens a connection to the Postgres database at `url`, runs all pending
    /// migrations, and returns the repository.
    pub async fn open(url: &str) -> Result<Self, PostgresReadRepositoryError> {
        let pool = PgPoolOptions::new()
            .connect(url)
            .await
            .map_err(PostgresReadRepositoryError::Connect)?;

        MIGRATOR
            .run(&pool)
            .await
            .map_err(PostgresReadRepositoryError::Migrate)?;
...
#[async_trait]
impl QueryRepository for PostgresReadRepository {
    #[instrument(
        skip_all,
        fields(
            db.system = "postgres",
    ...
    async fn execute(&self, query: &DicomQuery) -> Result<QueryPage, QueryRepositoryError> {
        ...
        let compiled = {
            let span = info_span!("postgres.query.compile");
        ...
        let rows = stmt
            .fetch_all(&self.pool)
            .instrument(info_span!("postgres.query.execute"))
            .await
            .map_err(|e| {
                Span::current().record("error.type", "sqlx");
                QueryRepositoryError::new(PostgresReadRepositoryError::Query(e).to_string())
            })?;
        let row_count = rows.len();
        Span::current().record("query.row_count", row_count);

        let materialize_span = info_span!(
            "postgres.query.materialize",
            ...
            sqlx::query_scalar("SELECT revision FROM read_model_state WHERE id = 1")
                ...
                .map_err(|e| {
                    QueryRepositoryError::with_source(
                        ...
                        PostgresReadRepositoryError::Query(e),
                    )
                })?;
...
#[async_trait]
impl RetrieveRepository for PostgresReadRepository {
    #[instrument(
        skip_all,
        fields(
            db.system = "postgres",
    ...
    async fn find_instances_for_patient(
        ...
        self.fetch_instance_refs(
            "SELECT i.study_instance_uid, i.series_instance_uid, i.sop_instance_uid, \
             ...
             WHERE st.patient_id = $1 AND i.object_key IS NOT NULL \
    ...
    async fn find_instances_for_study(
        ...
        self.fetch_instance_refs(
            "SELECT study_instance_uid, series_instance_uid, sop_instance_uid, sop_class_uid, \
             ...
             FROM instances \
             WHERE study_instance_uid = $1 AND object_key IS NOT NULL \
    ...
    async fn find_instances_for_series(
        ...
        self.fetch_instance_refs(
            "SELECT study_instance_uid, series_instance_uid, sop_instance_uid, sop_class_uid, \
                    transfer_syntax_uid, object_key, object_size_bytes \
             FROM instances \
             WHERE series_instance_uid = $1 AND object_key IS NOT NULL \
    ...
    async fn find_instances_for_study_series(
        ...
        let rows = sqlx::query(
            "SELECT study_instance_uid, series_instance_uid, sop_instance_uid, sop_class_uid, \
                    transfer_syntax_uid, object_key, object_size_bytes \
             FROM instances \
             WHERE study_instance_uid = $1 AND series_instance_uid = $2 AND object_key IS NOT NULL \
             ORDER BY sop_instance_uid",
        )
    ...
    async fn find_instance(
        ...
        let row = sqlx::query(
            "SELECT study_instance_uid, series_instance_uid, sop_instance_uid, sop_class_uid, \
                    transfer_syntax_uid, object_key, object_size_bytes \
             FROM instances \
             WHERE sop_instance_uid = $1 AND object_key IS NOT NULL",
        )
        .bind(uid.as_str())
            ...
            (Some(study_uid), Some(series_uid)) => {
                sqlx::query(
                    "SELECT study_instance_uid, series_instance_uid, sop_instance_uid, sop_class_uid, \
                            transfer_syntax_uid, object_key, object_size_bytes \
                     FROM instances \
                     WHERE study_instance_uid = $1 AND series_instance_uid = $2 AND sop_instance_uid = $3 \
            ...
            (Some(study_uid), None) => {
                sqlx::query(
                    "SELECT study_instance_uid, series_instance_uid, sop_instance_uid, sop_class_uid, \
                            transfer_syntax_uid, object_key, object_size_bytes \
                     FROM instances \
                     WHERE study_instance_uid = $1 AND sop_instance_uid = $2 AND object_key IS NOT NULL",
                )
                .bind(study_uid.as_str())
                .bind(sop_uid.as_str())
                .fetch_optional(&self.pool)
            ...
            (None, Some(series_uid)) => {
                sqlx::query(
                    "SELECT study_instance_uid, series_instance_uid, sop_instance_uid, sop_class_uid, \
                            transfer_syntax_uid, object_key, object_size_bytes \
                     FROM instances \
                     WHERE series_instance_uid = $1 AND sop_instance_uid = $2 AND object_key IS NOT NULL",
                )
                .bind(series_uid.as_str())
                .bind(sop_uid.as_str())
                .fetch_optional(&self.pool)
...
#[async_trait]
impl MetadataRepository for PostgresReadRepository {
    #[instrument(
        skip_all,
        fields(
            db.system = "postgres",
            ...
            RetrieveScope::Instance {
                ...
                sqlx::query(
                    "SELECT study_instance_uid, series_instance_uid, sop_instance_uid, \
                            sop_class_uid, attributes \
                     FROM instances \
                     WHERE study_instance_uid = $1 AND sop_instance_uid = $2",
                )
                .bind(study_uid.as_str())
                .bind(sop_instance_uid.as_str())
                .fetch_all(&self.pool)
            ...
            RetrieveScope::Instance {
                ...
                sqlx::query(
                    "SELECT study_instance_uid, series_instance_uid, sop_instance_uid, \
                            sop_class_uid, attributes \
                     FROM instances \
                     WHERE series_instance_uid = $1 AND sop_instance_uid = $2",
                )
                .bind(series_uid.as_str())
                .bind(sop_instance_uid.as_str())
                .fetch_all(&self.pool)
...
#[async_trait]
impl SyncReadModelWriter for PostgresReadRepository {
    #[instrument(
        skip_all,
        fields(
    ...
    async fn try_upsert_synced_object(
        ...
        let revision_update = sqlx::query(
            "UPDATE read_model_state \
             SET revision = revision + 1, updated_at_unix_ms = $1 \
             WHERE id = 1",
        )
        ...
        if revision_update.rows_affected() != 1 {
            return Err(PostgresReadRepositoryError::InternalError(
...
fn retrieve_query_error(err: sqlx::Error) -> RetrieveRepositoryError {
    Span::current().record("error.type", "sqlx");
    PostgresReadRepositoryError::Query(err).into()
}

fn materialize_instance_ref(row: PgRow) -> Result<InstanceRef, PostgresReadRepositoryError> {
...
fn materialize_instance_metadata(
    row: PgRow,
) -> Result<InstanceMetadata, PostgresReadRepositoryError> {
    let identity = DicomInstanceIdentity::new(
        parse_uid(&row, "study_instance_uid")?,
        parse_uid(&row, "series_instance_uid")?,
        parse_uid(&row, "sop_instance_uid")?,
    ...
    let attributes = row
        ...
        .map_err(|e| PostgresReadRepositoryError::RowRead("attributes".to_owned(), e))?;
    let attributes_json = attributes.to_string();

    Ok(InstanceMetadata::new(identity, attributes_json))
}

fn parse_uid<T>(row: &PgRow, column: &str) -> Result<T, PostgresReadRepositoryError>
where
    T: FromStr,
    T::Err: std::fmt::Display,
{
    let value = row
        ...
        .map_err(|e| PostgresReadRepositoryError::RowRead(column.to_owned(), e))?;
...
fn parse_optional_uid<T>(
    row: &PgRow,
    column: &str,
) -> Result<Option<T>, PostgresReadRepositoryError>
where
    T: FromStr,
    T::Err: std::fmt::Display,
{
...
fn parse_object_key(row: &PgRow, column: &str) -> Result<ObjectKey, PostgresReadRepositoryError> {
    let value = row
        .try_get::<String, _>(column)
        .map_err(|e| PostgresReadRepositoryError::RowRead(column.to_owned(), e))?;
...
fn parse_content_length(
    row: &PgRow,
    column: &str,
) -> Result<Option<u64>, PostgresReadRepositoryError> {
    row.try_get::<Option<i64>, _>(column)
        .map_err(|e| PostgresReadRepositoryError::RowRead(column.to_owned(), e))?
        .map(|value| {
            u64::try_from(value).map_err(|_| {
                PostgresReadRepositoryError::InvalidStoredRetrieveMetadata {
...
fn invalid_metadata(column: &str, err: impl std::fmt::Display) -> PostgresReadRepositoryError {
    PostgresReadRepositoryError::InvalidStoredRetrieveMetadata {
        column: column.to_owned(),
...
fn to_i64(field: &'static str, value: u64) -> Result<i64, PostgresReadRepositoryError> {
    i64::try_from(value)
        .map_err(|_| PostgresReadRepositoryError::IntegerOutOfRange { field, value })
}

fn read_error_kind(err: &PostgresReadRepositoryError) -> &'static str {
    match err {
        PostgresReadRepositoryError::Connect(_) => "raccoon_adapter_read_postgres::Connect",
        PostgresReadRepositoryError::Migrate(_) => "raccoon_adapter_read_postgres::Migrate",
        PostgresReadRepositoryError::Query(_) => "sqlx::Error",
        PostgresReadRepositoryError::RowRead(_, _) => "raccoon_adapter_read_postgres::RowRead",
        PostgresReadRepositoryError::InvalidPredicate(_) => {
            "raccoon_adapter_read_postgres::InvalidPredicate"
        }
        PostgresReadRepositoryError::InvalidStoredRetrieveMetadata { .. } => {
            "raccoon_adapter_read_postgres::InvalidStoredRetrieveMetadata"
        }
        PostgresReadRepositoryError::IntegerOutOfRange { .. } => {
            "raccoon_adapter_read_postgres::IntegerOutOfRange"
        }
        PostgresReadRepositoryError::InternalError(_) => {
            "raccoon_adapter_read_postgres::InternalError"
        }
    }
}
