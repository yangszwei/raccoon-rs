...
/// Converts a slice of raw Postgres rows into a [`QueryPage`].
...
) -> Result<QueryPage, PostgresReadRepositoryError> {
...
fn materialize_row(
    ...
    scope: QueryScope,
    registry: &AttributeRegistry,
) -> Result<QueryMatch, PostgresReadRepositoryError> {
    ...
    if needs_blob && compiled.includes_attributes {
        let blob: Option<serde_json::Value> = row.try_get("attributes").ok().flatten();
        if let Some(json) = blob
            .as_ref()
            .filter(|value| !value.as_object().is_some_and(|o| o.is_empty()))
    ...
    QueryMatch::new(attrs).map_err(|e| PostgresReadRepositoryError::InternalError(e.to_string()))
}
...
) -> Result<ResponseValue, PostgresReadRepositoryError> {
    ...
    match mapping.vr_class {
        ...
        VrClass::Integer => {
            let v: Option<i64> = row
                .try_get(col)
                .map_err(|e| PostgresReadRepositoryError::RowRead(col.to_string(), e))?;
            Ok(match v {
